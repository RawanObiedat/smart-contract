...
if submitted:
    if not student_id or not password:
        ...
        st.success(f"✅ Payment of {pay_amount} JOD received successfully!")

        # جدول الأقساط مع الحسابات
        months = 3
        ...
        payments = [pay_amount] + [0]*(months-1)
        statuses = ["Paid" if pay_amount > 0 else "Pending"] + ["Pending"]*(months-1)
        ...
        col2.metric("Paid Amount (JOD)", f"{pay_amount}")
        col3.metric("Remaining Amount (JOD)", f"{remaining_amount}")
        paid_installments = sum([1 for s in statuses if s == "Paid"])
        col4.metric("Paid Installments", f"{paid_installments} / {months}")

        # --- Progress bar ---
        progress = pay_amount / total_amount
        st.progress(progress)

        # --- Chart ---
        fig = px.pie(
            names=["Paid", "Remaining"],
            values=[pay_amount, remaining_amount],
