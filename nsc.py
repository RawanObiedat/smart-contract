...
with st.form("payment_form"):
    ...
    price_per_hour = 45
    total_amount = credit_hours * price_per_hour  # حساب المبلغ الإجمالي تلقائيًا
    st.markdown(f"**Total Tuition Fee:** {total_amount} JOD")

    pay_amount = st.number_input(
        "Enter Payment Amount (JOD)",
        min_value=1,
        max_value=total_amount,
        step=1,
        value=total_amount
    )
    st.caption("💡 يمكنك دفع المبلغ كاملًا أو دفعة أولى، وسيتم تقسيط الباقي تلقائيًا.")
